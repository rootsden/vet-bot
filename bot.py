...
print("🔵 Шаг 1: Импорты выполнены", flush=True)

from config import *
from database import *
from utils import *

print(f"🔵 Шаг 2: Конфиг загружен. ADMIN_VK_ID = {ADMIN_VK_ID}", flush=True)

# Проверка токена
if not VK_TOKEN:
    print("🔴 ОШИБКА: VK_TOKEN не задан!", flush=True)
else:
    print(f"🔵 Шаг 3: VK_TOKEN получен (первые 20 символов: {VK_TOKEN[:20]}...)", flush=True)
...
print("🔵 Шаг 5: Хранилища созданы", flush=True)
...
async def admin_panel(message: Message):
    print(f"🔵 /admin вызван от {message.from_id}", flush=True)
...
async def start_consultation_button(message: Message):
    print(f"🔵 Консультация от {message.from_id}", flush=True)
    user_id = message.from_id
...
async def forward_to_admin(message: Message):
    print(f"🔵 Сообщение от {message.from_id}", flush=True)
    user_id = message.from_id
    await message.answer(
...
async def start_command(message: Message):
    print(f"🔵 /start от {message.from_id}", flush=True)
    user_id = message.from_id
...
async def handle_messages(message: Message):
    print(f"🔵 ОБЩИЙ ОБРАБОТЧИК: сообщение от {message.from_id}: {message.text[:50] if message.text else '[без текста]'}", flush=True)
    user_id = message.from_id
...
if __name__ == "__main__":
    ...
    print(f"✅ Администратор: vk.com/id{ADMIN_VK_ID}", flush=True)
    print("🔵 Запуск init_db()...", flush=True)
    init_db()
    print("🔵 init_db() выполнен", flush=True)
    print("🔵 Запуск bot.run_forever()...", flush=True)
    bot.run_forever()
